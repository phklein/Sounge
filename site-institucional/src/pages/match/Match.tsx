...
export function Match() {
    ...
    const [newNotifications, setNewNotifications] = useState<NotificationSimpleResponseDto[]>()

    setInterval(() => {
        console.log('Atualizando notificações')

        UserRoute.checkNewMatches(id).then(res => {
    ...
    const swiped = (direction: any, nameToDelete: any, indexSwiped: number, matchId: number) => {
        if (direction === 'right') {
            if (matchList != null) {
                UserRoute.likeUser(id, matchId).then(res => {
                    if (res.status === 200) {
    ...
    const makeVisibleAndInvisible = (visible: boolean) => {
        const containerModalMatchs = document.getElementsByClassName('container-modal-match')
    ...
    useEffect(() => {
        ...
        const getMatchList = async (user: UserProfileResponseDto) => {
            ...
            if (user?.group !=  null || user?.leader == true) {
                ...
                await UserRoute.getMatchList(parametersMatchUsers).then(res => {
                    if (res.status === 200) {
                ...
                await GroupRoute.getMatchList(parametersMatchGroups).then(res => {
                    if (res.status === 200) {
    ...
    const returnContainerModalMatch = (notification: NotificationSimpleResponseDto) => {
            ...
            return (
                <div className="container-modal-match" id="modalNewMatchId">
                    <div className="modal-match">
                        ...
                        ></i>
                        <p>Sintonize-se!</p>
                        <p>Parece que {notification.sender.name} atende a todos os seus requisitos.</p>
                        ...
                        <button onClick={buttonHandler} name="init chat">Iniciar Conversa</button>
                        <button onClick={buttonHandler} name="view profile">Visualizar Perfil</button>
                    </div>
                </div>
            )
        } else {
                    ...
                    <div className="match-content">
                        <div className="match-nav">
                            <div className="match-li" onClick={() => replaceMatchContent('match')}>
                                <p>Matches</p><div className="number-match-nav">{contactList?.length || 0}</div>
                            </div>
                        </div>
                        {   
                        ...
                        matchList != null ?
                            matchList?.map((match, index) => {
                                return (
                                    ...
                                    >
                                        <div
                                            style={{ backgroundImage: `url(${match?.profilePic || 'https://i.pinimg.com/736x/c9/e3/e8/c9e3e810a8066b885ca4e882460785fa.jpg'})` }}
                                            className="card"
                                        >
                                            <h3>{match?.name}</h3>
                                        </div>
