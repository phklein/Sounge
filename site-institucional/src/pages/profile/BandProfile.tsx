...
import { Autocomplete, Box, Button, CircularProgress, ClickAwayListener, TextField } from "@mui/material"
...
const REOL_BAND_PROFILE_PAGE_MOCK: IBandPageData = {
	profileHighlight: {
		...
		avatarSrc: "https://i.pinimg.com/736x/5d/ba/bd/5dbabdc399dcd3c377bf28bf5b80ad80.jpg",
...
const BABYMETAL_BAND_PROFILE_PAGE_MOCK: IBandPageData = {
	profileHighlight: {
		...
		avatarSrc: "https://studiosol-a.akamaihd.net/uploadfile/letras/albuns/9/4/0/2/381521408793193.jpg",
		userInfo: { name: "BABYMETAL", description: "NÃO! Não é metal de bebe, seu burro 🐋" },
	},
	profileIntroMembers: [
		{
		...
		{
			imageSrc: "https://i.pinimg.com/originals/ac/60/c4/ac60c4a8383950f04fefaf3d0900192c.jpg",
			userId: 3,
			name: "Moa Kikuchi",
			role: "Vocal",
			leader: false,
...
const USERS_MOCK: ISearchUsers = {
	users: [
		{
			imageSrc: "http://pm1.narvii.com/7022/62ae42858fe3836a2f0a5e9c1b822346eb8cadber1-1516-1080v2_uhq.jpg",
			userId: 4,
			name: "Kradness",
			role: "Vocalista",
			leader: false,
		...
		{
			imageSrc: "https://cdns-images.dzcdn.net/images/artist/3c7e82a847df0e25fea009504ca45664/500x500.jpg",
			userId: 5,
			name: "DECO*27",
			role: "Produtor",
			leader: false,
...
const MOCK_GET_PROFILE_USER_DATA = (bandId: number) => {
	...
	return MOCK_DATABASE_USERS.find((band) => band.id === bandId)?.data || undefined
}
	...
	return (
		<li
			className={`profileBandWrapper ${
				isCurrentUser ? "profileBandWrapper--currentUser" : "profileBandWrapper--otherMember"
			}`}
		>
			<a className='profileBand'>
				...
				<div className='profileBandInfo'>
					<span className='profileBandInfoName'>
						{leader ? (
							<span>
								{name} <span className='profileBandInfoLeader'>{`(Líder da Banda)⭐`}</span>
							</span>
						) : (
							name
						)}
			...
			<Button disabled={disabled} variant='outlined' onClick={handleClick} color={isCurrentUser ? "error" : undefined}>
...
const BandProfile = () => {
	...
	const [bandMemberForm, setBandMemberForm] = useState({ member: "", role: "" })
	const [searchedUsers, setSearchedUsers] = useState<ISearchUsers | any>([])
	const [profileBandData, setProfileBandData] = useState<IBandPageData | any>(undefined)
	const [loadingProfileBandData, setLoadingProfileBandData] = useState(false)
	const [loadingProfileBandMember, setLoadingProfileBandMember] = useState(false)
	const [loadingProfileBandMemberSave, setLoadingProfileBandMemberSave] = useState(false)
	const [loadingProfileBandMemberDelete, setLoadingProfileBandMemberDelete] = useState(false)
	const autocompleteBandMemberValue = useDebounce(bandMemberSearch, 300)
	...
	const saveNewMember = async (selectedMember: any) => {
		try {
			...
			// Trocar pela request API de salvar o novo membro na banda
			setProfileBandData({
				...profileBandData,
				profileIntroMembers: [
	...
	useEffect(() => {
		...
		getProfileBandData(Number.parseInt(profilePageId[0]))
	}, [])
			...
			renderPage: () => (
				<>
					<ProfileIntro
						...
						handleClick={(userProfileId: number) => navigate("/profile/" + userProfileId)}
					/>
					<ProfilePost />
				</>
			),
				...
				<div className='registerBandMemberFormWrapper'>
					<div className='registerBandMemberForm'>
						<Autocomplete
							...
							isOptionEqualToValue={(option, value) => option.userId === value.userId || option.userId !== undefined}
							onInputChange={(_, name) => {
								setBandMemberSearch(name)
							}}
							onChange={(_, name) => {
								setBandMemberForm({ ...bandMemberForm, member: name || "" })
							}}
							renderInput={(params) => (
								<CustomTextField
									{...params}
									...
									InputProps={{
										...
										endAdornment: (
											<React.Fragment>
												{loadingProfileBandMember ? <CircularProgress color='inherit' size={20} /> : null}
						...
						<CustomTextField
							...
							onChange={(event) => setBandMemberForm({ ...bandMemberForm, role: event.target.value })}
						/>
						<CustomButton
							color='success'
							variant='contained'
							sx={{ width: "100px", "& .MuiCircularProgress-root": { color: "var(--green)" } }}
							onClick={() => saveNewMember(bandMemberForm)}
							disabled={!(bandMemberForm.member !== "" && bandMemberForm.role !== "")}
						>
							{loadingProfileBandMemberSave ? <CircularProgress size={24} /> : "Adicionar"}
						</CustomButton>
					</div>
					<div className='profileBandMembersList'>
						<h2>Integrantes</h2>
						<ul className='profileLists'>
							{profileBandData?.profileIntroMembers.map((member: any, index: number) => (
